  ...
  if (hook) {
    ...
    case 'opens':
      ...
      .then(function(challenges) {
        challenges = _.groupBy(challenges, 'state');
        response = '';

        _.each(challenges['Proposed'], function(challenge) {
          response += challenge.toString() + "\n";
        })

        _.each(challenges['Accepted'], function(challenge) {
          response += challenge.toString() + "\n";
        })

        if (response == '' ) {
          response = 'There are no open challenges right now - Challenge someone by typing `ping vs "player-name"`';
        }
    ...
    case 'register':
      if (params.length != 2) {
        res.json({ text: "Invalid params, use _foosbot register_." });
      } else {
        var promises = [];
      ...
      pong.findPlayer(hook.user_name).then(
        function (player) {
          if ((params[2] == 'double' || params[2] == 'doubles') && (params[4] == 'against' || params[4] == 'vs') && (params.length == 7)) {
            ...
            res.json({ text: "Invalid params, use _foosbot challenge <singles|doubles> <opponent|teammate> against <opponent> <opponent>_." });
          }
        },
        function (err) {
          res.json({ text: err.message + " Are you registered? Use _foosbot register_ first." });
        }
      );
      break;
    ...
    case 'accept':
      if (params.length != 2) {
        res.json({ text: "Invalid params, use _foosbot accept_." });
      } else {
    ...
    case 'decline':
      if (params.length != 2) {
        res.json({ text: "Invalid params, use _foosbot decline_." });
      } else {
    ...
    case 'chicken':
      if (params.length != 2) {
        res.json({ text: "Invalid params, use _foosbot chicken_." });
      } else {
    ...
    case 'lost':
      if (params.length != 2) {
        res.json({ text: "Invalid params, use _foosbot lost_." });
      } else {
    ...
    case 'rank':
      if ((params.length != 2) && (params.length != 3)) {
        res.json({ text: "Invalid params, use _foosbot rank <username>_." });
      } else {
    ...
    case 'reset':
      if (!process.env.ADMIN_SECRET) {
        ...
        res.json({ text: "Invalid secret. Use _foosbot reset <username> <secret>_." });
      } else {
    ...
    case 'new_season':
      if (!process.env.ADMIN_SECRET) {
        ...
        res.json({ text: "Invalid secret. Use _foosbot new_season <secret>_." });
      } else {
        pong.resetAll().then(
          function (result) {
    ...
    default:
      res.json({ text: "I couldn't understand that command. Use _foosbot help_ to get a list of available commands." });
      break;
    }
  }
};
